...
if __name__ == '__main__':
    ...
    if len(updated_info_haksa) is not 0: 
        ...
        # Sending message
        KensTelegramSendMessage(kens_bot, ['706386870'], KnsTelegramMakeMessageHaksa(updated_info_haksa))

    if len(updated_info_janghak) is not 0: 
        KnsWriteInfo(new_info_dict_janghak, ku_exported_filename_janghak)
        # Sending message
        KensTelegramSendMessage(kens_bot, ['706386870'], KnsTelegramMakeMessageJanghak(updated_info_haksa))

    if len(updated_info_guukjae) is not 0: 
        KnsWriteInfo(new_info_dict_guukjae, ku_exported_filename_guukjae)
        # Sending message
        KensTelegramSendMessage(kens_bot, ['706386870'], KnsTelegramMakeMessageGuukjae(updated_info_guukjae))

    if len(updated_info_haksaeng) is not 0: 
        KnsWriteInfo(new_info_dict_haksaeng, ku_exported_filename_haksaeng)
        # Sending message
        KensTelegramSendMessage(kens_bot, ['706386870'], KnsTelegramMakeMessageHaksaeng(updated_info_haksaeng))
    
    if len(updated_info_ilban) is not 0:
        KnsWriteInfo(new_info_dict_ilban, ku_exported_filename_ilban)
        # Sending message
        KensTelegramSendMessage(kens_bot, ['706386870'], KnsTelegramMakeMessageIlban(updated_info_ilban))
